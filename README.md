...
Різниця в запуску кожного із середовищ полягає в наступному:
 ...
 `
 "QUERY PLAN"
"Sort  (cost=34.03..34.94 rows=364 width=72) (actual time=0.234..0.255 rows=366 loops=1)"
"  Sort Key: ""createdAt"" DESC"
"  Sort Method: quicksort  Memory: 59kB"
"  ->  Seq Scan on ""order""  (cost=0.00..18.55 rows=364 width=72) (actual time=0.013..0.129 rows=366 loops=1)"
"        Filter: (""userId"" = 7)"
...
### planner after
"QUERY PLAN"
"Index Scan Backward using ""idx_order_user_createdAt"" on ""order""  (cost=0.27..25.71 rows=364 width=72) (actual time=0.026..0.152 rows=366 loops=1)"
"  Index Cond: (""userId"" = 7)"
"Planning Time: 0.036 ms"
"Execution Time: 0.175 ms"

Отже як видно то зменшився і запланований і фактичний час виконання при тому, що і в першому і в другому випадках обробляється однакова кількість записів - 366 фактично і 364 за статистикою

 # DataLoader працює, N + 1 зник
 Я добавив ProductDataLoader клас, в якому реалізував логіку DataLoader для сутності Product. Щоб продемонструвати, що він працює я створив клас OrderItemResolver і добавив поле-зв'язок productId_ для сутності OrderItem. Продемонструвати роботу DataLoader я зміг тільки саме на сутності OrderItem, бо саме ця сутність пов'язана з сутністю Product. Сутність Order, зв'язана з сутністю Product не безпосередньо, а через сутність OrderItem, тому продемонструвати роботу DataLoader я зміг лише на сутності OrderItem.
 Отже, коли виконати у graphql плейграунді подібний запит:
 ```
 query {
  orders(filter: {userId: 8}) {
    orderItems {
      id,
      product {
        id
 ...
 ```
 То в консолі з'явиться повідомлення схоже до цього: `Proof that DataLoader works: 10,8`. Тобто, наскільки я розумію, це показує, що DataLoader здійснює один запит з масивом id, а не N запитів для кожного OrderItem. Іншими словами, це повідомлення є доказом, що N + 1 дійсно зник
